...
function subscribeToSessionFallback<T>(
	key: string,
	fallback: T,
...
function createBrowserStorageAdapter<T>(
...
export async function readLegacySettingsFromLocal() {
	...
	const syncMode = isSyncMode(result.syncMode)
		? result.syncMode
...
export const localSavedUrlsStorage = createBrowserStorageAdapter<SavedUrl[]>(
	"local",
	"savedUrls",
	[],
);

export const savedUrlsCacheStorage = createBrowserStorageAdapter<SavedUrl[]>(
	"local",
	"savedUrlsCache",
	[],
);

export const settingsStorage = createBrowserStorageAdapter<AppSettings>(
	"sync",
	"settings",
	DEFAULT_SETTINGS,
);

export const sidebarViewStorage = createBrowserStorageAdapter<SidebarView>(
	"session",
	"sidebarView",
	DEFAULT_SIDEBAR_VIEW,
);
